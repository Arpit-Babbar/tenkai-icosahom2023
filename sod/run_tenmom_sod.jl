...
initial_value = Eq.sod_iv # choices - Eq.sod_iv, Eq.two_shock_iv, Eq.two_rare_iv, Eq.two_rare_vacuum_iv
...
limiter = setup_limiter_tvb(eq; tvbM = tvbM, beta = 1.1)
...
param = Parameters(grid_size, cfl, bounds, save_iter_interval, save_time_interval,
                   ...
                   saveto = joinpath(@__DIR__, "output_N$(degree)_$nx")
                   )
#------------------------------------------------------------------------------
